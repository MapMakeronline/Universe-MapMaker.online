/**
 * SearchToolbox - Main Search Window with 4 Tabs
 *
 * Tabs:
 * 1. Działki (Parcels) - Search by precinct and plot number
 * 2. Szczegółowe (Advanced) - Advanced search (TODO)
 * 3. Słowa kluczowe (Keywords) - Text search (TODO)
 * 4. Wyszukiwanie globalne (Global) - Mapbox geocoding (existing)
 *
 * Currently implemented: Działki + Wyszukiwanie globalne
 *
 * Documentation: SEARCH_DOCUMENTATION.md (lines 9-90)
 */
...
type TabValue = 'parcels' | 'advanced' | 'keywords' | 'global';

const SearchToolbox: React.FC<SearchToolboxProps> = ({ open, onClose, mapRef }) => {
  ...
  // Debounced search (for global tab)
  useEffect(() => {
    if (activeTab !== 'global') return; // Only search when global tab is active
  ...
  return (
    <Dialog
      ...
      maxWidth="sm"
      fullWidth
      keepMounted // Keep children mounted when closed (important for ParcelSearchTab)
      PaperProps={{
        sx: {
          borderRadius: '8px',
    ...
    >
      ...
      >
        ...
        {/* Tab 2: Szczegółowe (Advanced Search) - TODO */}
        {activeTab === 'advanced' && (
          <Box sx={{ p: 3, textAlign: 'center' }}>
            <FilterListIcon sx={{ fontSize: 48, color: 'text.disabled', mb: 2 }} />
            <Typography variant="body2" color="text.secondary">
              Wyszukiwanie zaawansowane - w budowie
            </Typography>
          </Box>
        )}

        {/* Tab 3: Słowa kluczowe (Keyword Search) - TODO */}
        {activeTab === 'keywords' && (
          <Box sx={{ p: 3, textAlign: 'center' }}>
            <TitleIcon sx={{ fontSize: 48, color: 'text.disabled', mb: 2 }} />
            <Typography variant="body2" color="text.secondary">
              Wyszukiwanie po słowach kluczowych - w budowie
            </Typography>
          </Box>
        )}

        {/* Tab 4: Wyszukiwanie globalne (Mapbox Geocoding) */}
        {activeTab === 'global' && (
          <>
            {/* Description */}
            <Box
